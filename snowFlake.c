...
int m_point(double, double, double);
void line(double **, double **, int);
int count_points(double * xs);
void bump();
...
int main()
{
    ...
    double * xs;
    double * ys;
    xs = (double *) malloc(sizeof(double) * 2 + 1);
    ys = (double *) malloc(sizeof(double) * 2 + 1);
...
int m_point(double a, double b, double m){
    double offset =  b - a; 
    offset =  offset * m;
    return a + offset;
}
...
void line(double ** xs, double ** ys, int maxDepth){
    double * bumpXs;
    double * bumpYs;
    int i = 0;
    double x;
    double y;
    bumpXs = (double *) malloc(sizeof(double) * 4);
    bumpYs = (double *) malloc(sizeof(double) * 4);

    x = *xs[0];
    printf("%lf", x);
    /*
    while(xs[i+1] > 0){
        //make new array for points in bump
        //calc 1/3 and 2/3 points 
        bumpXs[0]  = m_point(*xs[i], *xs[i+1], 1.0/3.0);
        bumpYs[0]  = m_point(*ys[i], *ys[i+1], 1.0/3.0);
        bumpXs[2]  = m_point(*xs[i], *xs[i+1], 2.0/3.0);
        bumpYs[2]  = m_point(*ys[i], *ys[i+1], 2.0/3.0);
    }
    */
    ...
    //make addressed passed point to new array
    *xs =  bumpXs;
}
